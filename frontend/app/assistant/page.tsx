...
export default function Home() {
    ...
    try {
      ...
      if (saved) {
        ...
        if (exp && exp * 1000 > Date.now() + 5000) {
          // Defer out of the effect body (avoids react-hooks/set-state-in-effect).
          const id = setTimeout(() => setSession(saved), 0);
  ...
  useEffect(() => {
    ...
    // verifyToken only reads stable setters; re-running on its identity would
    // needlessly re-render the Turnstile widget.
  ...
  async function handleAudio(blob: Blob) {
    try {
      ...
      if (tRes.status === 429) { setError(t.slowDown); setPhase("idle"); return; }
  ...
  async function playText(text: string, idx: number) {
    ...
    try {
      ...
      if (sRes.status === 429) { setError(t.slowDown); setPhase("idle"); setPlayingIdx(null); return; }
  ...
  return (
    <main className="page" dir={rtl ? "rtl" : "ltr"}>
      ...
      <section className="thread">
        ...
        {error && (
          <div className="bubble bubble--error" style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
